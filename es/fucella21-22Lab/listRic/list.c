...
void printListRicInner(struct node *n){
  outputItem(n->value);
  if(n->next != NULL)
    printListRicInner(n->next);
}

void printListRic(List l){
    printListRicInner(l->head);
}

int countItemRicInner(struct node *n, Item sear){
  int cont = 0;

  if(n->next !=  NULL)
    cont = countItemRicInner(n->next, sear);

  if(cmpItem(sear, n->value) == 0)
    ++cont;

  return cont;
}

int countItemRic(List l, Item a){
  return countItemRicInner(l->head, a);
}

Item searchItemRicInner(struct node *n, Item sear){
  if(cmpItem(n->value, sear) == 0)
    return n->value;
  else if(n->next != NULL)
    searchItemRicInner(n->next,  sear);
}

Item searchItemRic(List l, Item a){
  return searchItemRicInner(l->head, a);
}

void dealloclistRec(List l){
  struct node *tmp;
